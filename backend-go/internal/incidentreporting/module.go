...
func RegisterRoutes(p *gin.RouterGroup, h *Handler, db *pgxpool.Pool) {
	...
	h.db = db
	g := p.Group("/incident-reporting")
	g.GET("", middleware.RequirePermission(db, "THREAT_VIEW"), h.GetIncidentReports)
}
func (h *Handler) GetIncidentReports(c *gin.Context) {
	operational.ListIncidentStage(c, h.db, "incident reporting", []string{})
}
