...
func RegisterRoutes(p *gin.RouterGroup, h *Handler, db *pgxpool.Pool) {
	...
	h.db = db
	g := p.Group("/incident-learning")
	g.GET("", middleware.RequirePermission(db, "THREAT_VIEW"), h.GetIncidentLearning)
}
func (h *Handler) GetIncidentLearning(c *gin.Context) {
	operational.ListIncidentStage(c, h.db, "incident learning", []string{"RESOLVED", "CLOSED"})
}
