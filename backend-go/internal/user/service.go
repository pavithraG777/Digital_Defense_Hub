...
func (s *Service) CreateUser(
	...
	request.UserType = strings.ToUpper(
		strings.TrimSpace(request.UserType),
	)
...
func isValidUserType(userType string) bool {
	switch userType {
	case "INTERNAL", "EXTERNAL", "SERVICE_ACCOUNT":
		return true
	default:
		return false
	}
...
func (s *Service) ListUsers(
	...
	req.AccountStatus = strings.ToUpper(
		strings.TrimSpace(req.AccountStatus),
	)
...
func (s *Service) UpdateUser(
	...
	if req.OfficialEmail != nil {
		email := strings.ToLower(strings.TrimSpace(*req.OfficialEmail))

		if email == "" {
			return nil, errors.New("official email cannot be empty")
		}
	...
	if req.EmployeeCode != nil {
		value := strings.TrimSpace(*req.EmployeeCode)
	...
	if req.MiddleName != nil {
		value := strings.TrimSpace(*req.MiddleName)
	...
	if req.Designation != nil {
		value := strings.TrimSpace(*req.Designation)
		req.Designation = &value
	}

	if req.OfficialPhone != nil {
		value := strings.TrimSpace(*req.OfficialPhone)
