...
export function MfaSetupPage() {
  ...
  const [enrollmentRequired] = useState(
    () => !replacing || sessionStorage.getItem("ddh.authenticator-enrollment-required") === "true",
  );
  ...
  useEffect(() => {
    if (!replacing || !enrollmentRequired || automaticStartAttempted.current) return;
    automaticStartAttempted.current = true;
    void start("TOTP");
  }, [enrollmentRequired, replacing]);
  ...
  async function start(next: SetupMethod) {
    ...
    try {
      ...
      setTotp(null);
      setQrDataUrl("");
      setError(err instanceof Error ? err.message : "MFA setup could not be started.");
    }
  ...
  async function confirm(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (method === "EMAIL" && emailChallenge) {
        await api.post("/auth/mfa/email/enrollment/confirm", { mfa_challenge_id: emailChallenge.mfa_challenge_id, email_code: code });
        ...
        setCode("");
        return;
      } else if (method === "TOTP" && totp?.enrollment_token) {
        ...
        setTotp({ ...totp, recovery_codes: value.recovery_codes });
        sessionStorage.removeItem("ddh.authenticator-enrollment-required");
        setComplete(true);
        return;
  ...
  return <main className="auth-shell compact-auth-shell mfa-auth-shell"><section className="auth-panel auth-panel-centered"><form className="login-card mfa-card" onSubmit={confirm}>
    <div className="auth-card-heading"><span className="auth-card-logo"><ShieldCheck size={34} /></span><p className="eyebrow">{enrollmentRequired ? "FIRST-TIME MFA SETUP" : "MFA SECURITY"}</p><h2>{enrollmentRequired ? "Protect your account" : "Replace Google Authenticator"}</h2><p className="muted">{enrollmentRequired ? "Verify your email, then scan the Google Authenticator QR code once." : "Your current authenticator remains valid until the new QR code is verified."}</p></div>
    {!method && !complete && <div className="mfa-method-grid single-method">
      {enrollmentRequired && <button className="mfa-method" type="button" onClick={() => start("EMAIL")}><Mail size={22} /><span><strong>Send Email OTP</strong><small>Step 1 of 2: verify your official email</small></span></button>}
      {replacing && !enrollmentRequired && <button className="mfa-method" type="button" onClick={() => start("TOTP")}><Smartphone size={22} /><span><strong>Generate New QR</strong><small>Safely replace current setup</small></span></button>}
    </div>}
    ...
    {complete && totp && <div className="replacement-complete"><ShieldCheck size={42} /><h3>{enrollmentRequired ? "Authenticator enabled" : "Authenticator replaced"}</h3><p>Save these recovery codes securely. Each recovery code can be used only once.</p><div className="recovery-code-grid">{totp.recovery_codes.map((item) => <code key={item}>{item}</code>)}</div><button className="primary button-full" type="button" onClick={() => navigate(enrollmentRequired ? "/dashboard" : "/profile", { replace: enrollmentRequired })}>{enrollmentRequired ? "Continue to Dashboard" : "Continue to Profile"}</button></div>}
